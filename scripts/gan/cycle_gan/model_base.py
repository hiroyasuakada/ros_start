...
#
#         self.model_1 = nn.Sequential(
#             ConvLSTM_2(input_size=(128, 256), input_dim=3, hidden_dim=[64, 64], kernel_size=(3, 3), num_layers=2,
#                        batch_first=True, return_all_layers=True, cuda_use=True, device=device)
#         )
#
#         self.model_2 = nn.Sequential(
...
#
#     def forward(self, input_img):
#         layer_output_list, _ = self.model_1(input_img.to('cuda:1'))
#         out = self.model_2(layer_output_list[1]).squeeze()
#         return out.to('cuda:0')
#
#     @staticmethod
...
class Conv2dConv3dGenerator(nn.Module):
    def __init__(self, batch_size, window_size, step_size):
        super(Conv2dConv3dGenerator, self).__init__()
        ...
        self.model_3 = Reshape([-1, int(window_size / step_size), 256, 32, 64])
        ...
        # self.model_5 = Reshape([-1, 256, 32, 64])
        ...
        self.model_7 = Reshape([-1, int(window_size / step_size), 3, 128, 256])
    ...
    def forward(self, input_img):
        ...
        out = out.permute([0, 2, 1, 3, 4])
        out = out.contiguous().view([-1, 256, 32, 64])
        out = self.model_6(out)
        out = self.model_7(out)
        return out
...
# class FrameDiscriminator_B(nn.Module):
#
#     def __init__(self, batch_size, window_size, step_size):
#         super(FrameDiscriminator_B, self).__init__()
#
#         self.model_1 = Reshape([int(batch_size * window_size / step_size), 3, 128, 256])
#
#         self.model_2 = nn.Sequential(
...
#
    ...
    print(params_1)  # 4664833

    C2C3G = Conv2dConv3dGenerator(batch_size, window_size, step_size).to(device)
    real_or_not_2 = C2C3G(data['A'])
    print(real_or_not_2.shape)
    # パラメータカウント
    params_2 = 0
    for p in C2C3G.parameters():
        if p.requires_grad:
            params_2 += p.numel()

    print(params_2)  # 21993475
