...
class LSTMCycleGAN(object):
    ...
    def __init__(self, log_dir='logs_lstm_cyclegan', device='cuda:0', lr=0.0002, beta1=0.5, lambda_idt=5, lambda_A=10.0,
        ...
        self.netD_A_individual = individualDiscriminator_A(self.batch_size, self.window_size, self.step_size).to('cuda:0')
        self.netD_B_individual = individualDiscriminator_B(self.batch_size, self.window_size, self.step_size).to('cuda:1')
        ...
        self.optimizer_D_A_individual = torch.optim.Adam(self.netD_A_individual.parameters(), lr=self.lr, betas=(self.beta1, 0.999))
        self.optimizer_D_B_individual = torch.optim.Adam(self.netD_B_individual.parameters(), lr=self.lr, betas=(self.beta1, 0.999))
        self.optimizers = []
    ...
    def backward_D_A(self, seq_real_B, seq_fake_B):
        ...
        pred_real = self.netD_A_individual(seq_real_B)
        loss_D_real = self.criterionGAN(pred_real, True)

        # ドメインAから生成した偽物画像を入れたときは偽物と認識するほうがよい
        # fake_Bを生成したGeneratorまで勾配が伝搬しないようにdetach()する
        pred_fake = self.netD_A_individual(seq_fake_B.detach())
    ...
    def backward_D_B(self, seq_real_A, seq_fake_A):
        ...
        # 本物画像を入れたときは本物と認識するほうがよい
        pred_real = self.netD_B_individual(seq_real_A)
        loss_D_real = self.criterionGAN(pred_real, True)

        # 偽物画像を入れたときは偽物と認識するほうがよいdpkg エラー 修復
        pred_fake = self.netD_B_individual(seq_fake_A.detach())
        loss_D_fake = self.criterionGAN(pred_fake, False)
    ...
    def save_network(self, network, network_label, epoch_label):
        ...
        # GPUに戻す
        if network is self.netG_A or network is self.netD_A_seq or network is self.netD_A_individual:
            network.to('cuda:0')
        else:
            network.to('cuda:1')
